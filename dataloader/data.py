...
def get_transform(args):
    ...
    config_train = {'scale': args.scale,'color_jitter': args.color_jitter,'input_size': (3, 224, 224), 'interpolation': 'bicubic', 'mean': args.mean, 'std': args.std, 'crop_pct': 0.9, 'is_training': True}
    config_test = {'input_size': (3, 224, 224), 'interpolation': 'bicubic', 'mean': args.mean, 'std': args.std, 'crop_pct': 0.9, 'is_training': False}
    TRANSFORM_TRAIN = create_transform(**config_train)
    TRANSFORM_TEST = create_transform(**config_test)
...
def get_dataset(args):
    ...
    for t in range(args.ntasks):
        ...
        for c in cls_id:
            ...
            # if dataset_name.startswith('C10-'):
            #     idx = idx[:500]
        ...
        if dataset_name.startswith('T-'):
            ...
            # Extract data and targets from train_ using the indices
            train_.data = []
